...
class Activity3 {
    ...
    public static int halfTimes(int num){
        int res = 0; // number of times num can be halved
        while(num > 1){
            ++res; // each time num is divided by 2 increase res
            num = num/2;
        }
        return res;
    }
    ...
    public static void guessGame(){
        int rnum = ((int) (Math.random() * (100 - 1))) + 1; // random number between 1 and 100
        boolean gameOver = false; // variable to tell if its game over
        System.out.print("Guess the number: ");
        while(!gameOver){ // keep playing until user guesses the right answer
            int guess = inpt.nextInt(); // user input
            if(guess == rnum){ // user won
